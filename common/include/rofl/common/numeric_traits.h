...
struct FloatTraits<float> {
    ...
    static UnsignedType toInt(const FloatType& floatVal) {
        union {
            FloatType f;
            UnsignedType i;
        } floatBits;
        floatBits.f = floatVal;
        return floatBits.i;
    }

    static FloatType dividePow2(const FloatType& floatVal, const IntegerType& exp2) {
        IntegerType mantissa, exponent;
        bool sign;
        getMantissaExpSignF(floatVal, mantissa, exponent, sign);
        if (exponent < exp2) {
            return 0.0f;
        } else {
            mantissa = mantissa & (MANTISSA_MASK >> (MANTISSA_BITS - exponent + exp2));
            return setMantissaExpSignF(mantissa, exponent, sign);
        }
    }
};
